...
@if (auth()->user()->roles[0]->name == 'admin')
            ...
            <h1 class="text-3xl font-semibold text-center text-gray-800 capitalize lg:text-4xl dark:text-black">
                {{strtoupper( __('Continuing Education')) }}</h1>
                  ...
                  {{--  <div
                        ...
                        @if (auth()->user()->roles[0]->name == 'admin')
                            <div class="flex mt-3 -mx-2">
                                ...
                                <a href="{{ route('education.destroy', $education) }}" type="button"
                        ...
                        @endif
                            ...
                            <div class="flex justify-center xl:w-1/2">
                                <img class="h-80 " src="{{ $education->image }}" alt="imagen">
                            </div>
                            ...
                            <div class="flex flex-col items-center mt-6 xl:items-start xl:w-1/2 xl:mt-0">
                                <h2 class="text-3xl font-bold tracking-tight text-gray-800 xl:text-4xl dark:text-white">
                                   {{ $education->name_courses}}
                                </h2>
                    
                                <p class="block max-w-2xl mt-4 text-xl text-gray-500 dark:text-gray-300">{{$education->fecha_courses}} </p>
                                
                                <div class="mt-6 sm:-mx-2">
                                    <p class="text-gray-500 dark:text-gray-300">{{$education->description}}</p>
                                    <p class="text-gray-500 dark:text-gray-300">{{$education->cel_phone}}</p>
                                    <p class="text-gray-500 dark:text-gray-300">{{$education->place}}</p>
                                </div>
                                @if (auth()->user()->roles[0]->name == 'admin')
                            <div class="flex mt-3 -mx-2">
                                <a href="{{ route('education.edit', $education) }}" type="button"
